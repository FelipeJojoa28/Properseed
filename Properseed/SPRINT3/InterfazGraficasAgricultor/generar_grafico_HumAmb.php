...
$mysqli = new mysqli($host, $user, $pw, $db);
    ...
    $sql = "SELECT TRUNCATE(AVG(Humedad_ambiente),2) as HUMA_PRO FROM Parametros WHERE Id_dispositivo = '$id_disp' AND Fecha >= '$fecha_ini' AND Fecha <= '$fecha_fin' GROUP BY HOUR(Hora) ORDER BY Hora";
	$huma = mysqli_query($mysqli,$sql);
	$huma = mysqli_fetch_all($huma,MYSQLI_ASSOC);
	$huma = json_encode(array_column($huma, 'HUMA_PRO'),JSON_NUMERIC_CHECK);
...
$(function () { 
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Humedad del aire por hora'
        },
        xAxis: {
            title: {
                text: 'Hora'
...
<div class="contenedor">
	<br/>
	<h2 class="text-center">Promedios de humedad del aire entre: <?php echo $fecha_ini?> y <?php echo $fecha_fin?></h2>
